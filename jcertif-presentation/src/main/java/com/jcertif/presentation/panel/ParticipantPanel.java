 ...
 * @author Douneg
 */
public class ParticipantPanel extends MasterDetailsPanel<ParticipantAction> implements
	...
	private Button addPropositionPresentation = new Button("+ Proposition Presentation",
			(Button.ClickListener) this);
	private Button addCeduleParticipant = new Button("+ Cedule Participant",
			(Button.ClickListener) this);
	private Button manageProfilU = new Button("Editer Profil Utilisateur",
			(Button.ClickListener) this);
	private Button detailsCedule = new Button("Details", (Button.ClickListener) this);
	private Button detailsPropositionPresentation = new Button("Details",
			(Button.ClickListener) this);
	...
	public ParticipantPanel(ParticipantAction participantAction) {
		...
		propositionPresentationTable = new PropositionPresentationTable(
				propositionPresentationAction);

		allDetails = new HashSet<Component>();

		allButtons = new HashSet<Button>();
		allButtons.add(detailsCedule);
		Layout detail = buildDetailPanel("Cedule", "Liste des Cedules", ceduleParticipantTable,
				allButtons);
		allDetails.add(detail);

		allButtons = new HashSet<Button>();
		allButtons.add(detailsPropositionPresentation);
		detail = buildDetailPanel("Proposition Presentation",
				"Liste des Proposition Presentations", propositionPresentationTable, allButtons);
		allDetails.add(detail);
		...
		Layout all = buildMainPanel("Participants", "Liste des participants", participantTable,
				allButtons, allDetails);
	...
	public void valueChange(ValueChangeEvent event) {
		...
		if (property == participantTable) {
			...
			ceduleParticipantAction.getPrincipalContainer().loadData(
					participant.getCeduleParticipants());
			propositionPresentationAction.getPrincipalContainer().loadData(
					participant.getPresentations());
