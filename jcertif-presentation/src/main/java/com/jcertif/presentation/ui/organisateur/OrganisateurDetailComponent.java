...
public class OrganisateurDetailComponent extends Panel {
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(OrganisateurDetailComponent.class);
		...
		for (Participant participant : participantList) {
			...
			if (isFirsTime) {
				htmlLayout.addComponent(
						new Label(Msg.get("ui.organisateur.title")), "caption");
				htmlLayout.addComponent(
						new Label(Msg.get("ui.organisateur.description")),
						"captionDetail");
			}
			...
			if (participant.getProfilUtilisateur() != null
				...
				embedded.setStyleName(UIStyle.PHOTO_ORGANISATEUR);
