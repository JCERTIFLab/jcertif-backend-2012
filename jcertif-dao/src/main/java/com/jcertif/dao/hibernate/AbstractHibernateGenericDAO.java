 ...
 */
public abstract class AbstractHibernateGenericDAO<T, PK extends Serializable> implements
		GenericDAO<T, PK> {

	/**
	 * The logger.
	 */
	private static final Logger LOGGER = LoggerFactory.getLogger(AbstractHibernateGenericDAO.class);
	/**
	...
	public List<T> findAll() {
		...
		final List<T> result = this.getCurrentSession().createCriteria(this.persistentClass).list();
		long end = System.currentTimeMillis();
		LOGGER.debug("findAll() persistentClass={}, result={}, time={}", new Object[] {
				this.persistentClass, result.size(), (end - start) });
		return result;
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<T> findAllWithSort(final String sortedPropertyName, final boolean ascending) {
		final Criteria criteria = this.getCurrentSession().createCriteria(this.persistentClass);
		...
		LOGGER.debug("findAllWithSort() persistentClass={}, result={}", this.persistentClass,
				result.size());
		return result;
	}
	...
	public T findById(final PK id) {
		final T result = (T) this.getCurrentSession().get(this.persistentClass, id);
		LOGGER.debug("findById() with id={} returns {}", new Object[] { this.persistentClass, id,
				result });
		return result;
	}
	...
	public T getReference(final PK id) {
		LOGGER.debug("Loading peristentClass={},id={}", this.persistentClass, id);
